import rejectSeries from "../lib/rejectSeries";
...
describe("promizr.rejectSeries()", () => {

    test("should returns the filtered array", async () => {
        const spy = jest.fn(num => num % 2 === 0);

        const res = await rejectSeries(LIST, num => timeout(num).then(() => spy(num)));
    ...
    test("should call each provided methods with given values in series", async () => {
        ...
        await rejectSeries(LIST, (num, index, list) => timeout(num).then(() => spy(num, index, list)));
    ...
    test("should reject if an iterator throws", async () => {
        ...
        await expect(rejectSeries(LIST, spy))
            .rejects.toBe(err);
    });
    ...
    test("should stop if an iterator throws", async () => {
        ...
        await expect(rejectSeries(STOP_LIST, iterator))
            .rejects.toBe(err);
