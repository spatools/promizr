...
describe("promizr.filter()", () => {

    test("should returns the filtered array", async () => {
        const spy = jest.fn(num => num % 2 === 1);

        const res = await filter(LIST, num => timeout(num).then(() => spy(num)));
    ...
    test("should call each provided methods with given values in parallel", async () => {
        ...
        await filter(LIST, (num, index, list) => timeout(num).then(() => spy(num, index, list)));
    ...
    test("should stop if an iterator throws", async () => {
        ...
        await expect(filter(STOP_LIST, iterator))
            .rejects.toBe(err);
