...
describe("promizr.each()", () => {

    test("should returns void", async () => {
        const spy = jest.fn();

        const res = await each(LIST, spy);
    ...
    test("should call each provided methods with given values in parallel", async () => {
        ...
        await each(LIST, (num, index, list) => timeout(num).then(() => spy(num, index, list)));
    ...
    test("should stop if an iterator throws", async () => {
        ...
        await expect(each(STOP_LIST, iterator))
            .rejects.toBe(err);
